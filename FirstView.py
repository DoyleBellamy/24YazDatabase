...
def main_page():
    ...
    #st.image("https://encrypted-tbn3.gstatic.com/images?q=tbn:ANd9GcTgTJielSTEbhb--_10WvYIEmaYlFOZ2d7jE3-3lrfy94rczcw1", caption="Welcome to the Veterinary System", use_column_width=300)
    st.text_input("Mail Adresi", key="login_email")
...
def register_page():
    ...
    if st.button("Geri"):
        ...
        st.experimental_rerun()
    st.text_input("Mail Adresi", key="register_email")
    st.text_input("Şifre", type="password", key="register_password")
    st.text_input("İsim", key="register_name")
    st.text_input("Soyisim vs.", key="register_surname")
    if st.button("Kayıt Ol"):
        st.session_state.prev_page = st.session_state.page
        st.session_state.page = "User Main"
        st.experimental_rerun()
